...
var zz = {
	...
	RequesFc: function (data) {
		...
		var request_fc_nub = nm.RequestFunctionSequenceNumber();
		var return_fc_nub = nm.RequestFunctionSequenceNumber();
		var fail_fc_nub = nm.RequestFunctionSequenceNumber();
		var complete_fc_nub = nm.RequestFunctionSequenceNumber();
		if (RequestData.fail) {
		...
		if (RequestData.success) {
			...
			return_fc.innerHTML =
				...
				"')\n}";
			if (RequestData.fail) {
				nm.DestructionEvent(fail_fc_nub);
			}
		...
		if (RequestData.complete) {
			...
			nm.FunctionExecution(complete_fc_nub);
		}
		nm.DestructionEvent(request_fc_nub);
		requestSetTime = setTimeout(() => {
			if (document.getElementById(return_fc_nub)) {
				nm.DestructionEvent(return_fc_nub);
				console.warn("Error:Request to " + RequestData.url + " failed");
				if (RequestData.fail) {
					nm.FunctionExecution(fail_fc_nub);
				}
